...
class PauzeTodayConditionViewModel @Inject constructor(
    ...
    uiState = BaseUiState(
        data = TodayConditionState(conditionQuestions = todayConditionQuestions)
    )
) {
    fun selectAnswer(choiceIndex: Int) {
        if (uiState.value.data.isSubmitting) return
    ...
    private fun submitTodayCondition() {
        ...
        updateData { it.copy(isSubmitting = true, submissionError = null) }
        ...
        launch<CreateTodayConditionResult>(
            ...
            block = {
                try {
                    ...
                    updateData { it.copy(isSubmitting = false) }
                }
            }
        )
    }
